...
def read_winogender_dataset():
    df = pd.read_csv(f"./data/winogender/winogender.gpt2.csv", index_col=0)
    return df
        ...
        for method in methods:
            ...
            for layer in range(0, layerwise_contributions.shape[0]):
                ...
                if layer == 0:
                    ...
                    token_layer_contribs = torch.matmul(layerwise_contributions[layer, -1, token].cpu().detach(),
                                                        contributions_mix_alti[layer - 1])
                    alti_logit_layer_token = torch.cat([alti_logit_layer_token, token_layer_contribs.unsqueeze(0)],
                                                       dim=0)
